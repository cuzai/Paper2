...
class Embedding(torch.nn.Module):
    def __init__(self, data_info, label_encoder_dict, d_model, temporal_cols, img_cols, nlp_cols, patch_size):
        ...
        for col in self.img_cols:
            self.embedding_dict[col] = ImgEmbedding(patch_size, d_model)
...
class TemporalRemain(torch.nn.Module):
    ...
    def forward(self, data_dict, idx_dict, padding_mask_dict, remain_rto, device):
        ...
        temporal_block_revert_padding_mask = torch.ones_like(padding_mask_dict["temporal_padding_mask"]).unsqueeze(-1).repeat(1, 1, total_num_modality).to(device)
...
class Revert(torch.nn.Module):
    def __init__(self, d_model, temporal_cols, img_cols, nlp_cols):
        ...
        temporal_mask_token = torch.nn.Parameter(torch.rand(1, d_model))
        img_mask_token = torch.nn.Parameter(torch.rand(1, d_model))
        nlp_mask_token = torch.nn.Parameter(torch.rand(1, d_model))

        # Temporal
        self.temporal_revert = TemporalRevert(temporal_mask_token, temporal_cols)
        
        # Img, Nlp
        self.non_temporal_revert_dict = torch.nn.ModuleDict()
        for col in img_cols:
            self.non_temporal_revert_dict[col] = NonTemporalRevert(img_mask_token)
        for col in nlp_cols:
            self.non_temporal_revert_dict[col] = NonTemporalRevert(nlp_mask_token)
