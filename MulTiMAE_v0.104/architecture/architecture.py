...
class IndivPosEmbedding(torch.nn.Module):
    ...
    def __init__(self, config, d_model, dropout):
        ...
        # temporal_pos_enc_2d = torch.from_numpy(get_2d_sincos_pos_embed(embed_dim=d_model, grid_size=config.MAX_SEQ_LEN+config.PRED_LEN, cls_token=False))
        # self.temporal_pos_enc_2d = torch.nn.Parameter(temporal_pos_enc_2d.reshape(config.MAX_SEQ_LEN+config.PRED_LEN, config.MAX_SEQ_LEN+config.PRED_LEN, -1), requires_grad=False)
        temporal_pos_enc_2d = torch.from_numpy(get_2d_sincos_pos_embed(embed_dim=d_model, grid_size=900, cls_token=False))
        self.temporal_pos_enc_2d = torch.nn.Parameter(temporal_pos_enc_2d.reshape(900, 900, -1), requires_grad=False)
...
class IndividualEncoding(torch.nn.Module):
    ...
    def forward(self, data_dict, padding_mask_dict, device):
        ...
        # print(padding_mask_dict["target_fcst_mask"].shape)
        for mod in self.layers_dict["temporal"]:
            x, attn_weight = mod(x, src_key_padding_mask=padding_mask_dict["target_fcst_mask"])
...
class TotalDecoding(torch.nn.Module):
    ...
    def forward(self, data_dict, padding_mask_dict, device, mode="pre_train", mode2="revert"):
        ...
        # if mode == "fine_tuning": self.temporal_cols = ["sales"]
        # Temporal decoding
        for col in self.temporal_cols:
            ...
            memory_padding_mask = torch.cat([padding_mask_dict["temporal_padding_mask"] if i=="temporal" else padding_mask_dict[f"{i}_{mode2}_padding_mask"] for i in ["temporal"]+self.img_cols+self.nlp_cols], dim=1)
        ...
        # self.img_cols, self.nlp_cols = [], []
        # Static decoding
        for col in self.img_cols+self.nlp_cols:
            tgt = modemb_dict[col]
            tgt_padding_mask = padding_mask_dict[f"{col}_{mode2}_padding_mask"]
            tgt_padding_mask = torch.where(tgt_padding_mask==1, 0, -torch.inf)
...
class MBAEEncoder(torch.nn.Module):
    def __init__(self, config, label_encoder_dict,
        ...
        super().__init__()
        self.config = config()
        self.linear = torch.nn.Linear(config.d_model["encoder"], config.d_model["encoder"])
        ...
        # 5. Total encoding
        self.total_encoding = TotalDecoding(config, modality_embedding, config.d_model["encoder"], config.d_ff["encoder"], config.nhead["encoder"], config.dropout, config.activation, config.num_layers["encoder"], self.linear)
        # 7. Revert
    ...
    def forward(self, data_dict, idx_dict, padding_mask_dict, device):
        ...
        # 4. Individual encoding
        indiv_encoding_dict, indiv_weight_dict = self.indinv_encoding(remain_dict, padding_mask_dict, device)
        # 5. Total encoding
        total_encoding_dict, total_weight_dict = self.total_encoding(indiv_encoding_dict, padding_mask_dict, device, mode="remain")
...
class MBAEDecoder(torch.nn.Module):
    ...
    def forward(self, encoding_dict, padding_mask_dict, device, mode):
        # 0. To decoder dimension
        ...
        # 3. Individual encoding
        indiv_encoding_dict, indiv_weight_dict = self.indinv_encoding(decoding_dict, padding_mask_dict, device)
        # elif mode == "fine_tuning":
        #     indiv_encoding_dict = {"temporal" if key=="temporal_block" else f"{key}":val for key, val in decoding_dict.items()}
        # 4. Total decoding
        total_decoding_dict, total_weight_dict = self.total_decoding(indiv_encoding_dict, padding_mask_dict, device, mode)
        # 5. Output
        output_dict = self.output(total_decoding_dict, mode)
...
class MaskedBlockAutoencoder(torch.nn.Module):
    ...
    def forward(self, data_input, device, mode="pre_train"):
        ...
        decoding_dict, decoding_weight_dict = self.decoder(encoding_dict, padding_mask_dict, device, mode)
        # print(decoding_weight_dict["sales"].shape)
