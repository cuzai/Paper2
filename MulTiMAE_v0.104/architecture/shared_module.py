...
class MultiheadBlockAttention(torch.nn.Module):
    ...
    def forward(self, query, key, value, padding_mask):
        ...
        # padding_mask = padding_mask.unsqueeze(1).unsqueeze(-1).expand(-1, logits.shape[1], -1, logits.shape[-1])
        padding_mask = padding_mask.unsqueeze(1).expand(-1, logits.shape[1], -1).unsqueeze(-1)
        # print(mask.shape)
