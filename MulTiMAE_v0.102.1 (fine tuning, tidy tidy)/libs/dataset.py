...
#     transforms.RandomRotation(degrees=45),   # Rotates the image by up to 45 degrees
#     transforms.ColorJitter(brightness=0.2),  # Randomly changes brightness
# ])
...
def collate_fn(batch_li, data_info):
    ...
    for col in img_cols:
        ...
        img_data = transform_img(img_raw)
...
class CustomLabelEncoder(BaseEstimator, TransformerMixin):
    def fit(self, x, y=None):
        ...
        self.val_to_idx_dict = {val:n for n, val in enumerate(nuique_value)}
        self.idx_to_val_dict = {val:key for key, val in self.val_to_idx_dict.items()}
        return self
