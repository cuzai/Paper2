...
class Decoder(torch.nn.Module):
    ...
    def forward(self, data_dict, padding_mask_dict):
        ...
        for col in self.total_cols:
            tgt_dict, tgt_key_padding_mask_dict, memory_dict, memory_key_padding_mask_dict= self.get_tgt_memory(col, data_dict, padding_mask_dict)
    ...
    def get_tgt_memory(self, col, data_dict, padding_mask_dict):
        ...
        if len(static_memory_li) > 0:
            ...
            static_key_padding_mask_li = [padding_mask_dict[f"{key}_revert_padding_mask"] for key in data_dict.keys() if key!=col and key in self.img_cols+self.nlp_cols]
...
class Output(torch.nn.Module):
    def __init__(self, data_info, d_model, label_encoder_dict, temporal_cols, img_cols, nlp_cols, patch_size):
        super().__init__()
