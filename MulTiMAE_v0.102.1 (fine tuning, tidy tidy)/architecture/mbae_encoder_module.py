...
class ImgEmbedding(torch.nn.Module):
    def __init__(self, global_token, patch_size, d_model, is_from_pretrained):
        super().__init__()
...
class NlpEmbedding(torch.nn.Module):
    def __init__(self, d_model, is_from_pretrained):
        super().__init__()
        self.is_from_pretrained = is_from_pretrained
...
class Embedding(torch.nn.Module):
    def __init__(self, data_info, label_encoder_dict, d_model, temporal_cols, img_cols, nlp_cols, patch_size, is_from_pretrained):
        super().__init__()
        ...
        for col in self.img_cols:
            self.embedding_dict[col] = ImgEmbedding(self.global_token, patch_size, d_model, is_from_pretrained)
        for col in self.nlp_cols:
            self.embedding_dict[col] = NlpEmbedding(d_model, is_from_pretrained)
...
class Encoder(torch.nn.Module):
    ...
    def forward(self, temporal_block, img_dict, nlp_dict, padding_mask_dict):
        src_dict, src_key_padding_mask_dict = self.get_src(temporal_block, img_dict, nlp_dict, padding_mask_dict)
    ...
    def get_src(self, temporal_block, img_dict, nlp_dict, padding_mask_dict):
        ...
        if len(img_dict) or len(nlp_dict):
            ...
            ### Padding mask
            img_padding_mask_li = [padding_mask_dict[f"{col}_remain_padding_mask"] for col in img_dict.keys()]
            nlp_padding_mask_li = [padding_mask_dict[f"{col}_remain_padding_mask"] for col in nlp_dict.keys()]
...
class TemporalRevert(torch.nn.Module):
    def __init__(self, mask_token, temporal_cols, is_from_pretrained):
        super().__init__()
        self.is_from_pretrained = is_from_pretrained
...
class NonTemporalRevert(torch.nn.Module):
    def __init__(self, mask_token, is_from_pretrained):
        super().__init__()
        self.mask_token = mask_token
        self.is_from_pretrained = is_from_pretrained
...
class Revert(torch.nn.Module):
    def __init__(self, d_model, temporal_cols, img_cols, nlp_cols, is_from_pretrained):
        super().__init__()
        
        mask_token = torch.nn.Parameter(torch.rand(1, d_model))

        # Temporal
        self.temporal_revert = TemporalRevert(mask_token, temporal_cols, is_from_pretrained)
        
        # Img, Nlp
        self.non_temporal_revert_dict = torch.nn.ModuleDict()
        for col in img_cols + nlp_cols:
            self.non_temporal_revert_dict[col] = NonTemporalRevert(mask_token, is_from_pretrained)
