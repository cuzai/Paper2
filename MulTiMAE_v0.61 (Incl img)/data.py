...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Img data
        img_col_name = self.data_info.modality_info["img"]
        if len(img_col_name) > 0:
            img_col_name = img_col_name[0]
            img_path = data[img_col_name].values[0]
            img_raw = Image.open(img_path).convert("RGB")

            result_dict["img"] = self.transform(img_raw, return_tensors="pt", train=False)["pixel_values"].squeeze()
            result_dict["img_raw"] = img_raw
...
def collate_fn(batch_li, data_info):
    ...
    # Img data
    if len(data_info.modality_info["img"]) > 0:
        result_dict["img"] = torch.stack([batch["img"] for batch in batch_li])
        result_dict["img_raw"] = [batch["img_raw"] for batch in batch_li]
...
class DataInfo():
    ...
    def _check_modality(self):
        ...
        num_img = len(self.modality_info["img"])
        num_modality = num_target + num_temporal + num_img
        print(num_modality)

        # Processing info
        num_scaling_cols = len(self.processing_info["scaling_cols"])
        num_embedding_cols = len(self.processing_info["embedding_cols"])
        num_img_cols = len(self.processing_info["img_cols"])
        num_processing = num_scaling_cols + num_embedding_cols + num_img_cols
