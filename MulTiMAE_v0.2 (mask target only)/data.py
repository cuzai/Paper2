...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, data, data_info, remain_rto_target, remain_rto_temporal, is_train=True, label_encoder_dict=None):
        self.data_info, self.remain_rto_target, self.remain_rto_temporal, self.is_train = data_info, remain_rto_target, remain_rto_temporal, is_train
    ...
    def _process_img(self, data):
        ...
        for col in self.data_info.processing_info["img_cols"]:
            ...
            result_dict[f"{col}_img_input"] = self.transform(img_raw, return_tensors="pt", train=False)["pixel_values"].squeeze(0)
        return result_dict

    def _get_mask(self, data, remain_rto):
        num_remain = int(len(data) * remain_rto) if self.is_train else -90
    ...
    def _mask_temporal(self, data, col_li, remain_rto):
        result_dict = {}
        for col in col_li:
            remain_idx, masked_idx, revert_idx, remain_padding_mask, masked_padding_mask, revert_padding_mask = self._get_mask(data[col], remain_rto)
    ...
    def __getitem__(self, idx):
        ...
        # Mask temporal
        data_input.update(self._mask_temporal(data_input, self.data_info.modality_info["target"], self.remain_rto_target))
        if self.is_train:
            data_input.update(self._mask_temporal(data_input, self.data_info.modality_info["temporal"], self.remain_rto_temporal))
        else:
            data_input.update(self._mask_temporal(data_input, self.data_info.modality_info["temporal"], 1))
...
def collate_fn(batch_li, data_info):
    ...
    for col in data_info.modality_info["target"] + data_info.modality_info["temporal"]:
        ...
        temporal_remain_padding_mask = [batch[f"{col}_remain_padding_mask"] for batch in batch_li]
        temporal_masked_padding_mask = [batch[f"{col}_masked_padding_mask"] for batch in batch_li]
        temporal_revert_padding_mask = [batch[f"{col}_revert_padding_mask"] for batch in batch_li]
        ...
        result_dict[f"{col}_remain_padding_mask"] = torch.nn.utils.rnn.pad_sequence(temporal_remain_padding_mask, batch_first=True)
        result_dict[f"{col}_masked_padding_mask"] = torch.nn.utils.rnn.pad_sequence(temporal_masked_padding_mask, batch_first=True)
        result_dict[f"{col}_revert_padding_mask"] = torch.nn.utils.rnn.pad_sequence(temporal_revert_padding_mask, batch_first=True)
...
class DataInfo():
    ...
    def get_num_modality(self, data_info):
        ...
        num_modality = num_target + num_temporal + others + num_img_path
        return num_modality
