...
class Dataset(torch.utils.data.Dataset):
    ...
    def transform_embeddings(self, data):
        result_dict = {}
        for col in self.config.embedding_cols:
            result_dict[f"{col}"] = self.label_encoder_dict[col].transform(data[col].values)
        return result_dict

    def scale_scalables(self, data, hist_len):
        result_dict = {}
        for col in self.config.scaling_cols:
            scaler = self.config.processing_info["scaling_cols"][col]()
            scaler.fit(data[col].iloc[:hist_len].values.reshape(-1,1))
            result_dict[f"{col}"] = scaler.transform(data[col].values.reshape(-1,1))
    ...
    def __getitem__(self, idx):
        ...
        hist_len = data.shape[0]

        # Sampling
        if self.mode == "fine_tuning":
            total_length = data.shape[0]
            start_idx = np.random.randint(0, total_length-self.config.LEAST_HIST_LEN-self.config.PRED_LEN)
            end_idx = np.random.randint(start_idx+self.config.LEAST_HIST_LEN+self.config.PRED_LEN, total_length)
            
            data = data.iloc[start_idx:end_idx]
            hist = data[:-self.config.PRED_LEN]
            fcst = data[-self.config.PRED_LEN:]
        ...
        ### Scaling data
        scaled_data = self.scale_scalables(data, hist_len)
...
def collate_fn(batch_li, config):
    ...
    for col in config.temporal_cols:
        ...
        data = [torch.from_numpy(batch[f"{col}"]).to(tensor_type) for batch in batch_li]
