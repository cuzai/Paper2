...
class RawData():
    def __init__(self):
        self.trans_col = ["t_dat", "customer_id", "article_id", "price"]
    ...
    def get_raw_data(self, is_exist=False):
        if not is_exist:
...
class Preprocess():
    ...
    def _make_sales(self, data):
        ...
        data["meaningful_size"] = data.groupby(["article_id"], as_index=False)["sales"].transform("count")
        return data
    ...
    def _expand_date(self, data):
        ...
        data["full_size"] = data.groupby(["article_id"], as_index=False)["sales"].transform("count")
    ...
    def main(self):
        ...
        data = df_with_holiday.sort_values(["article_id", "t_dat"]).reset_index(drop=True)
