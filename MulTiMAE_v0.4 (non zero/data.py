...
class Dataset(torch.utils.data.Dataset):
    ...
    def _apply_remain(self, data, valid_idx, num_remain):
        noise = np.random.rand(len(data[valid_idx]))
        shuffle_idx = np.argsort(noise, axis=0)
        # shuffle_idx = valid_idx[shuffle_idx]
    ...
    def _apply_target_remain(self, data):
        ...
        num_remain = int(len(data) * self.remain_rto["target"])

        for col in target_cols:
            value = data[col].values

            valid_idx = np.arange(len(value)) if not self.is_non_zero else np.where(value > 0)[0]
            remain_idx, masked_idx, revert_idx, remain_padding_mask, masked_padding_mask, revert_padding_mask = self._apply_remain(value, valid_idx, num_remain)
    ...
    def _apply_temporal_remain(self, data, valid_idx):
        ...
        num_remain = int(len(data) * self.remain_rto["temporal"])

        for col in target_cols:
            value = data[col].values
            remain_idx, masked_idx, revert_idx, _, _, _ = self._apply_remain(data[col].values, valid_idx, num_remain)
