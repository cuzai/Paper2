...
class RemainMasking(torch.nn.Module):
    def __init__(self, temporal_cols, img_cols, nlp_cols, temporal_modality_embedding, global_token, remain_rto, patch_size, d_model, dropout):
        super().__init__()
    ...
    def get_temporal_remain_block(self, data_dict, remain_dict, idx_dict, remain_rto, device):
        # Make temporal block
        temporal_dict = {}
        for modality_idx, key in enumerate(self.temporal_cols):
            val = data_dict[key]
            ...
            temporal_dict[key] = val + modality
...
class MBAEEncoder(torch.nn.Module):
    def __init__(self, config, label_encoder_dict, global_token, mask_token, temporal_modality_embedding, modality_embedding):
        ...
        # 2. Remain masking
        self.remain_masking = RemainMasking(["global"]+config.temporal_cols, config.img_cols, config.nlp_cols, temporal_modality_embedding, global_token, config.remain_rto, config.patch_size, config.d_model["encoder"], config.dropout)
        ...
        # 4. Total encoding
        self.total_encoding = TotalEncoding(config.temporal_cols, config.img_cols, config.nlp_cols, modality_embedding, config.d_model["encoder"], config.d_ff["encoder"], config.nhead["encoder"], config.dropout, config.activation, config.num_layers["encoder"])
        # 6. Revert
...
class MaskedBlockAutoencoder(torch.nn.Module):
    def __init__(self, config, label_encoder_dict):
        ...
        num_total_modality = len(["temporal"] + config.img_cols + config.nlp_cols)
        modality_embedding = torch.nn.Embedding(num_total_modality, config.d_model["encoder"])
        # dec_modality_embedding = torch.nn.Embedding(num_total_modality, config.d_model["decoder"])

        self.encoder = MBAEEncoder(config, label_encoder_dict, global_token, mask_token, temporal_modality_embedding, modality_embedding)
        self.decoder = MBAEDecoder(config, label_encoder_dict, global_token, mask_token, temporal_modality_embedding, modality_embedding)
