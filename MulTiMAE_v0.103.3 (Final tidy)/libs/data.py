...
def load_dataset(is_test_mode, is_new_rawdata, config, mode, verbose):
    ...
    torch.save(train_dataset_test, f"src/{mode}_train_dataset_test")
    torch.save(train_dataset, f"src/{mode}_train_dataset")
...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, data, config, mode):
        ...
        self.img_augmentation = transforms.Compose([transforms.RandomResizedCrop(224),
                                                    transforms.ToTensor(),
                                                    transforms.Normalize(mean=[0.7623358, 0.74334157, 0.738284], std=[0.27400097, 0.2852157, 0.28155997]),
    ...
    def __getitem__(self, idx):
        ...
        ### Temporal mask
        temporal_padding_mask = np.ones(data[self.config.target_col].shape).squeeze()
        target_fcst_mask = np.ones(data[self.config.target_col].shape)
        if self.mode == "fine_tuning":
            target_fcst_mask[-self.config.PRED_LEN:] = 0
        ...
        for col in self.config.nlp_cols:
            ...
            nlp_padding_mask = nlp_token["attention_mask"][:, :-1]
...
def collate_fn(batch_li, config):
    ...
    result_dict["target_fcst_mask"] = torch.nn.utils.rnn.pad_sequence(temporal_padding_mask, batch_first=True)
