...
class Transformer(torch.nn.Module):
    ...
    def _data_to_gpu(self, data_input_dict, device):
        ...
        padding_mask_dict[f"temporal_remain_padding_mask"] = data_input_dict[f"temporal_remain_padding_mask"].to(device)
        padding_mask_dict[f"temporal_masked_padding_mask"] = data_input_dict[f"temporal_masked_padding_mask"].to(device)
        padding_mask_dict[f"temporal_revert_padding_mask"] = data_input_dict[f"temporal_revert_padding_mask"].to(device)
    ...
    def _mask_others_remain(self, dynamic_embedding_dict, idx_dict, remain_rto, device):
        ...
        others_remain_data, others_idx = self.others_remain(others_data, self.others_remain_pos_emb, remain_rto["cat"], device)
    ...
    def _mask_img_remain(self, img_embedding_dict, idx_dict, remain_rto, device):
        ...
        target_cols = self.data_info.modality_info["img"]
        for col in target_cols:
            img_remain_dict[f"{col}_img_input"], img_idx_dict = self.img_remain_dict[f"{col}_img_input"](img_embedding_dict[f"{col}_img_input"], self.img_remain_pos_enc, remain_rto, col, device)
    ...
    def _get_padding_mask(self, others_remain_data, padding_mask_dict, img_remain_dict, mode, device):
        ...
        # Temporal
        for col in self.data_info.modality_info["target"] + self.data_info.modality_info["temporal"]:
            mask = padding_mask_dict[f"temporal_{mode}_padding_mask"]
    ...
    def _revert_temporal(self, temporal_data, idx_dict, padding_mask_dict, device):
        ...
        temporal_revert_dict = {}
        for col in target_cols:
            temporal_revert_dict[col] = self.temporal_revert_dict[col](temporal_data[col], idx_dict[f"{col}_revert_idx"], self.temporal_revert_pos_enc, padding_mask_dict[f"temporal_remain_padding_mask"], device)
