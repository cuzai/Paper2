...
class MBAEEncoder(torch.nn.Module):
    def __init__(self, data_info, label_encoder_dict, temporal_cols, img_cols, nlp_cols,
        ...
        # self.full_encoder = torch.nn.TransformerEncoder(torch.nn.TransformerEncoderLayer(d_model["encoder"], nhead, d_ff["encoder"], dropout, activation, batch_first=True, norm_first=True), num_layers["encoder"])
    ...
    def forward(self, data_input, remain_rto, device):
        ...
        # #############################
        # # Alternative Encoder Start #
        # flattened = temporal_block_remain.reshape(temporal_block_remain.shape[0], -1, temporal_block_remain.shape[-1])
        # flattened_padding_mask = padding_mask_dict["temporal_block_remain_padding_mask"].reshape(temporal_block_remain.shape[0], -1)

        # for col in self.img_cols:
        #     flattened = torch.cat([flattened, img_remain_dict[col]], dim=-2)
        #     flattened_padding_mask = torch.cat([flattened_padding_mask, padding_mask_dict[f"{col}_remain_padding_mask"]], dim=-1)
        # for col in self.nlp_cols:
        #     flattened = torch.cat([flattened, nlp_remain_dict[col]], dim=-2)
        #     flattened_padding_mask = torch.cat([flattened_padding_mask, padding_mask_dict[f"{col}_remain_padding_mask"]], dim=-1)
        
        # flattened_padding_mask = torch.where(flattened_padding_mask==1, 0, -torch.inf)
        
        # flattened = self.full_encoder(flattened, src_key_padding_mask=flattened_padding_mask)
        # flattened = self.norm2(flattened)
        ...
        # temporal_encoding_block, img_encoding_dict, nlp_encoding_dict = temporal_block_remain, img_remain_dict, nlp_remain_dict
        # encoding_weight_dict = None
        # # Alternative Encoder End #
        # #############################

        # 6. Revert
        revert_dict = self.revert(temporal_encoding_block, img_encoding_dict, nlp_encoding_dict, idx_dict, padding_mask_dict)
        
        flattened = torch.stack([val for key, val in revert_dict.items() if key in ["global"]+self.temporal_cols], dim=-2)
...
class MBAEDecoder(torch.nn.Module):
    def __init__(self, data_info, label_encoder_dict, temporal_cols, img_cols, nlp_cols, 
        ...
        self.decoder = Decoder(["global"]+temporal_cols, img_cols, nlp_cols, d_model["decoder"], nhead, d_ff["decoder"], dropout, activation, num_layers["decoder"])
    ...
    def forward(self, posmod_emb_dict, idx_dict, padding_mask_dict, device):
        ...
        return output_dict, cross_attn_weight_dict_dict
...
class MaskedBlockAutoEncoder(torch.nn.Module):
    ...
    def forward(self, data_input, remain_rto, device):
        ...
        decoding_dict, cross_attn_weight_dict_dict = self.mbae_decoder(posmod_emb_dict, idx_dict, padding_mask_dict, device)

        return decoding_dict, data_dict, idx_dict, padding_mask_dict, cross_attn_weight_dict_dict
