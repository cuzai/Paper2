...
class ImgRemain(torch.nn.Module):
    ...
    def forward(self, col, data, idx_dict, padding_mask_dict, remain_rto, device, data_dict):
...
class Remain(torch.nn.Module):
    ...
    def forward(self, data_dict, idx_dict, padding_mask_dict, remain_rto, device):
        ...
        img_remain_dict = {}
        for col in self.img_cols:
            img_remain, idx_dict, padding_mask_dict = self.img_remain_dict[col](col, data_dict[col], idx_dict, padding_mask_dict, remain_rto["img"], device, data_dict)
...
class EncoderLayer_(torch.nn.Module):
    def __init__(self, d_model, nhead, d_ff, dropout, activation):
        super().__init__()
    ...
    def forward(self, src_dict, src_key_padding_mask_dict):
        ...
        flatten_li = []
        temporal_length = 0
        for key, val in src_key_padding_mask_dict.items():
            if key == "temporal":
                temporal_block_shape = val.shape
                temporal_flattened = val.view(temporal_block_shape[0], -1)
                temporal_length = temporal_flattened.shape[1]
                flatten_li.append(temporal_flattened)
                
            elif key == "static":
                flatten_li.append(val)
        
        src_key_padding_mask = torch.cat(flatten_li, dim=1)
        ...
        temporal_length = src_dict["temporal"].shape[1] * src_dict["temporal"].shape[2]
        temporal = x[:, :temporal_length, :].reshape(src_dict["temporal"].shape)
        
        static = x[:, temporal_length:, :]
        
        
        return {"temporal":temporal, "static":static}, attn_weight
    ...
    def _sa_block(self, src_dict, src_key_padding_mask_dict):
        attn_output, attn_weight = self.self_attn(src_dict, src_dict, src_dict, src_key_padding_mask_dict)
...
class NonTemporalRevert(torch.nn.Module):
    ...
    def forward(self, col, data, idx_dict, padding_mask_dict):
        # Split global token 
        global_token = data[:, :1, :]
        valid_data = data[:, 1:, :]

        # Replace paddings to Mask token
        remain_padding_mask = padding_mask_dict[f"{col}_remain_padding_mask"].unsqueeze(-1).repeat(1, 1, valid_data.shape[-1])
        remain_padding_mask = remain_padding_mask[:, 1:, :]
        valid_data = torch.where(remain_padding_mask==1, valid_data, self.mask_token)
...
class Revert(torch.nn.Module):
    ...
    def forward(self, temporal_block_encoding, img_encoding_dict, nlp_encoding_dict, idx_dict, padding_mask_dict):
        ...
        for key, val in img_encoding_dict.items():
            img_revert_dict[key] = self.non_temporal_revert_dict[key](key, val, idx_dict, padding_mask_dict)
        
        # Nlp
        nlp_revert_dict = {}
        for key, val in nlp_encoding_dict.items():
            nlp_revert_dict[key] = self.non_temporal_revert_dict[key](key, val, idx_dict, padding_mask_dict)
